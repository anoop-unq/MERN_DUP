...
//     try {
//         const existingGuest = await userModel.findOne({ isGuest: true });

//         if (existingGuest) {
//             return res.status(200).json({
//                 success:false,
//                 message: 'Guest profile already exists',
//                 guestId: existingGuest._id,
//                 existingGuest
//             });
//         }

//         const guestProfile = new userModel({
//             name: `Guest ${Date.now()}`,
//             isGuest: true,
//         });

//         await guestProfile.save();
//     //     const token = jwt.sign({id:guestProfile},process.env.SECRET_KEY,{expiresIn:"7d"});
//     //         console.log("Setting token:", token);
//     //         res.cookie('token',token,{
//     //         httpOnly:true,
//     //         secure:process.env.NODE_ENV === 'production',
...
export const createGuest = async (req, res) => {
  try {
    ...
    const existingGuest = await Guest.findOne({ isGuest: true });

    if (existingGuest) {
      // If the guest profile exists, return success and the guest ID
      return res.status(200).json({
        success: true,
        message: 'Guest profile already exists',
        guestId: existingGuest._id,
    ...
    // If guest profile doesn't exist, create a new one
    const guestProfile = new Guest({
      name: `Guest ${Date.now()}`,
      isGuest: true,
    });
    ...
    const token = jwt.sign({ id: guestProfile._id }, process.env.SECRET_KEY, { expiresIn: "7d" });
    res.cookie('token', token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      sameSite: "None",
...
export const getGuest = async (req, res) => {
    try {
        const guestId = req.query.guestId; // Access the guestId from the query string

        if (!guestId) {
            return res.status(400).json({ error: 'Guest ID is required' }); // Check if guestId is passed
        }

        // Find the guest profile by guestId (_id)
        const guestProfile = await Guest.findById(guestId);

        if (!guestProfile || !guestProfile.isGuest) {
            return res.status(404).json({ message: 'Guest profile not found' }); // Profile not found or it's not a guest
        }

            const token = jwt.sign({id:guestId},process.env.SECRET_KEY,{expiresIn:"7d"});
            console.log("Setting token:", token);
            res.cookie('token',token,{
            httpOnly:true,
            secure:process.env.NODE_ENV === 'production',
        ...
        // Send the guest profile in the response
        res.json({
            success:true,
        ...
        console.error(error);
        res.status(500).json({ error: 'Error fetching guest profile' }); // Return a 500 error if an exception occurs
    }
};
...
export const getUserData = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            // If the user is found in the userModel
            return res.json({
                success: true,
                userData: {
                    name: user.name,
        ...
        // If the user is not found in userModel, check the guestModel
        const guest = await Guest.findById(userId);
        
        if (guest) {
            // If the user is found in the guestModel
            return res.json({
                success: true,
                // userData: {
                //     name: guest.name,
                //     isAccountVerified: false,  // Assuming guests don't have verified accounts
                // },
            });
        }

        // If the user is not found in both models
        return res.status(400).json({ success: false, message: "User not found!" });
