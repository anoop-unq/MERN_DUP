...
const HomePage = () => {
  ...
  const { guest, realGuest,guestLogout,userdata } = useContext(AppContext);
  ...
  useEffect(() => {
    const fetchNote = async () => {
      try {
        ...
        console.log(receivedData, "From frontend HomePage.Jsx");
    ...
    <>
      <nav className="navbar">
        <div className="logo">
          {!userdata ? (
            <div>
              <button onClick={handleGuestLogout} className="bg-blue-500 text-white py-1 px-3 rounded-md text-sm font-medium shadow-sm hover:bg-blue-600 hover:scale-105 transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50 w-full sm:w-auto m-0">
                Logout
              </button>
            </div>
          ) : (
        ...
        <>
          <div className="empty-state-card">
            ...
            <p className="empty-description">
              You're all caught up! Start adding tasks to stay organized and
              productive.
            </p>
            <Link to="/create">
